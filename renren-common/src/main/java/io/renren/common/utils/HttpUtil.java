...
import io.renren.common.utils.ServiceResult;
...
public class HttpUtil {
    ...
    public static ServiceResult get(String url, String charset) {
        ...
        LogUtils.info(HttpUtil.class, "调用HttpClinet服务URL, url = {0}", url);
        ...
        try {
            ...
            if (statusCode == HttpStatus.SC_OK) {
                ...
                if (StringUtils.equalsIgnoreCase(IS_BYTE_DATA, charset)) {
                    Date end = new Date();
                    LogUtils.info(HttpUtil.class, "调用HttpClinet服务完成,请求结果 result = byte[{0}] 执行时间 {1} ms", responseBody.length, end.getTime() - start.getTime());
                    return result.setData(responseBody);
                }
                String response = new String(responseBody, Charset.forName(charset));
                LogUtils.info(HttpUtil.class, "调用HttpClient返回结果responseBody = {0}", response);

                result.setData(JSON.parse(response));
                Date end = new Date();
                LogUtils.info(HttpUtil.class, "调用HttpClinet服务完成,请求结果 result = {0} 执行时间 {1} ms", result.getError(), end.getTime() - start.getTime());
            } else {
                LogUtils.error(HttpUtil.class, "请求出错: " + getMethod.getStatusLine(), null);
            }
        } catch (IOException e) {
            LogUtils.error(HttpUtil.class, "调用HttpClient时发生异常", e);
        } finally {
    ...
    public static ServiceResult post(String url, String json, NameValuePair[] pairs, String param, String charset) {
        ...
        try {
            ...
            LogUtils.info(HttpUtil.class, "调用HttpClient参数 json = {0}", json);
            ...
            if (statusCode == HttpStatus.SC_OK) {
                ...
                LogUtils.error(HttpUtil.class, "请求出错: " + postMethod.getStatusLine(), null);
            }

        } catch (UnsupportedEncodingException e) {
            LogUtils.error(HttpUtil.class, "调用远程服务失败", e);
